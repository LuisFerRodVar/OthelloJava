/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package modelo;

import estructuras.Tablero;
import java.util.ArrayList;

/**
 * Esta clase genera un bot Loro
 *
 * @author luisf
 */
public class Loro extends Bot {

    public Loro() {
        super("LORO");
    }

    /**
     * Este metodo realiza las jugadas que un Loro puede pensar, basicamente
     * consiste en tomar todas las jugadas posibles, evaluarlas y realizar
     * alguna de las que mas material consiga en la posicion
     *
     * @param jugada Es la lista de jugadas posibles en la posicion
     * @return Retorna la jugada realizada por el Loro
     */
    @Override
    public String realizarJugada(ArrayList<String[]> jugada) { //1
        Tablero tablero1 = getTablero(); //2
        ArrayList<String[]> jugadas; //1
        if (jugada != null) { //1
            jugadas = jugada; //1
        } else {
            jugadas = tablero1.jugadasPosibles(); //2
        }

        ArrayList<String> jugadasRealizables = new ArrayList(); //2
        if (jugadas.isEmpty()) { //1
            return "nada"; //1
        }
        int mayor = Integer.valueOf(jugadas.get(0)[1]); //4
        for (int i = 0; i < jugadas.size(); i++) { //4n + 1
            if (Integer.valueOf(jugadas.get(i)[1]) > mayor) { //4n
                mayor = Integer.valueOf(jugadas.get(0)[1]); //4
            }
        }
        for (int i = 0; i < jugadas.size(); i++) { //4n + 1
            if (Integer.valueOf(jugadas.get(i)[1]) == mayor) { //4n
                jugadasRealizables.add(jugadas.get(i)[0]); //3
            }
        }
        int rangoMayor = jugadasRealizables.size() - 1; //3
        int numero = (int) (Math.random() * (rangoMayor - 0 + 1)); //5
        tablero1.realizarJugada(jugadasRealizables.get(numero)); //2
        return jugadasRealizables.get(numero); //2
    }
}
